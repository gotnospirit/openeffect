...
class LookingAround implements IEffect
{
    ...
    public function init(w : Int, h : Int) : Void
    {
        ...
        for (x in 0...w * 2)
        {
            ...
            for (y in 0...h * 2)
            {
                distance[x][y] = Math.ceil(ratio * texture_height / Math.sqrt((x - w) * (x - w) + (y - h) * (y - h))) % texture_height;
                angle[x][y] = Math.ceil(0.5 * texture_width * Math.atan2(y - h, x - w) / Math.PI);
            }
        }
    }
    ...
    public function render(frame : Bitmap) : Void
    {
        ...
        var shift_look_x : Int = Math.ceil(w / 2 + (w / 2 * Math.sin(animation)));
        var shift_look_y : Int = Math.ceil(h / 2 + (h / 2 * Math.sin(animation * 2.0)));
        ...
        for (y in 0...h)
        {
            for (x in 0...w)
            {
                ...
                bm.setPixel(x, y, texture.getPixel32(
                    Math.ceil(distance[x + shift_look_x][y + shift_look_y] + shift_x) % texture_width,
                    Math.ceil(angle[x + shift_look_x][y + shift_look_y] + shift_y) % texture_height
                ));
            }
        }
        bm.unlock();
